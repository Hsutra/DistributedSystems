...
public class Lab4_1 {
    ...
    public static void main(String[] args) throws MPIException {
        ...
        localMatrix = new int[n * n / size + 1];
        localVector = new int[n / size];

        long startTime = System.nanoTime();

        MPI.COMM_WORLD.Scatter(matrixToArray(matrix), 0, n * n / size, MPI.INT, localMatrix, 0, n * n / size, MPI.INT, 0);
        MPI.COMM_WORLD.Scatter(vector, 0, n / size, MPI.INT, localVector, 0, n / size, MPI.INT, 0);

        partialResult = multiplyMatrixVector(matrixFrom1DArray(localMatrix, n / size, n), localVector);

        int[] globalResult = new int[n];
        MPI.COMM_WORLD.Gather(partialResult, 0, n / size, MPI.INT, globalResult, 0, n / size, MPI.INT, 0);
